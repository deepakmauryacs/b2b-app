        ...
        if (data.length) {
            ...
            data.forEach(function (cat) {
                ...
                var html = '<div class="col-md-3 col-sm-6 mb-3">' +
                    '<a href="' + url + '" class="card shadow-sm category-card" style="background-color: #e6f2ff;" data-slug="' + cat.slug + '">' +
                    '<div class="card-body py-3">' +
    ...
    if (data.length) {
        data.forEach(function (prod) {
            var imageSection = prod.product_image
                ? '<div class="position-relative" style="height:180px; overflow:hidden; margin: 20px 20px 0 20px;">' +
                      '<img src="' + prod.product_image + '" class="card-img-top h-100 w-100" style="object-fit: cover;" alt="' + prod.product_name + '">' +
                  '</div>'
                : '<div class="d-flex align-items-center justify-content-center border-bottom" style="height:180px; background-color: #e7f2ff; margin: 20px 20px 0 20px;">' +
                      '<span class="fw-bold text-muted text-center">' + prod.product_name + '</span>' +
                  '</div>';

            var card = '<div class="col-md-3 col-sm-6 mb-4">' +
                '<div class="card h-100 border shadow-sm">' +
                    imageSection +
                    '<div class="card-body d-flex flex-column justify-content-between text-center" style="min-height: 130px;">' +
